...
export default function AdvancedSearchBar({ trips = [], onSearch }) {
  ...
  return (
    <div className="relative w-full flex justify-center z-[200]">
      ...
      <form onSubmit={handleSubmit}
        ...
        {/* Location input opens popup */}
        <div className="flex items-center gap-2 flex-1 cursor-pointer" onClick={()=>setOpen(true)}>
          <MapPin size={18} className="text-orange-600"/>
          <input 
            readOnly
            value={query}
            placeholder="Search Stay Type or City..."
            className="bg-transparent outline-none text-sm md:text-[15px] w-full"
          />
        </div>

        <Mic size={18} className="text-orange-600 cursor-pointer hidden md:block" onClick={voice}/>
        ...
        <button 
          ...
          rounded-full px-5 md:px-7 py-2 text-xs md:text-sm">
          LET'S GO
        </button>
      </form>
      ...
      {/* ===================== POPUP PANEL ===================== */}
      {open&&(
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-start p-5 z-[500] overflow-y-auto">
          <div ref={panelRef}
            className="bg-white rounded-3xl shadow-xl w-full max-w-3xl p-6 max-h-[85vh] overflow-y-auto">
    ...
    <div className="flex flex-wrap gap-2">
      {data.map(v=>(
        <span key={v} onClick={()=>pick(v)}
          className="px-3 py-1 bg-gray-100 hover:bg-orange-300 rounded-full text-[12px] cursor-pointer transition">
          {v}
        </span>
      ))}
    </div>
